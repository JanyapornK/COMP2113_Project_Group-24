...
// Global variables for player's character and current scenario
...
void startNewGame() {
    ...
    while (gameInProgress) {
        ...
        if (currentScenario == 0) 
        {
            ...
            case '1':
                ...
                if (dynamic_cast<rebel*>(playerCharacter) == nullptr) 
                {
                    ...
                    {
                        // Game over
                        cout << "Game over. You were caught while looking for the key." << endl;
        ...
        else if (currentScenario == 1) 
        {
            ...
            switch (choice) {
            case '1':
                // Scenario 1: Maze to enter professor's office
        ...
        else if (currentScenario == 3) 
        {
            ...
            switch (choice) {
            case '1':
                // Proceed to Scenario 4: Maze to escape the building
...
void saveGame() 
{
    // Base case: return if no character selected
    ...
    if (outfile) {
        ...
        // Saving special ability and ability info
...
void loadGame() 
{
    ...
    string name, specialAbility, abilityInfo;
    int health, attack, defence, intelligence;
    int loadedScenario;
    
    // Read the character data from savegame.txt
    infile >> name >> health >> attack >> defence >> intelligence >> loadedScenario;
    infile.ignore();
    ...
    {
        cout << "Unknown character type in the save file." << endl;
    }

    // Correct the stats and scenario as saved
